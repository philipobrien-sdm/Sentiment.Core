...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .synth-title { font-serif italic; font-size: 14px; color: #FFFFFF; }
    .synth-body {
      padding: 20px;
      background: #FFFDF9;
...
<body>
  ...
  <main class="content">
    ...
    <section id="tab-syntheses" class="tab-panel">
      <div class="card">
        <div class="card-title">Saved AI Synthesis Reports &amp; Critiques (${synthesisHistory.length})</div>
        <p style="color: #6B7280; margin-bottom: 20px; font-size: 12px;">
          Includes qualitative LLM cluster syntheses, stakeholder power-interest priority critiques, and neighborhood semantic analyses generated during application sessions.
        </p>

        ${synthesisHistory.length === 0 ? '<div style="background: #FFFDF9; border: 1px solid #E5E3DF; padding: 24px; text-align: center; color: #888;">No saved synthesis reports in this export snapshot.</div>' : ''}

        ${synthesisHistory.map((synth, idx) => `
          <div class="synth-item">
            <div class="synth-header" onclick="toggleSynthBody('synth-body-${idx}')">
              <div>
                <div class="synth-title">📜 ${escapeHtml(synth.title)}</div>
                <div style="font-size: 10px; color: #FCD34D; font-family: monospace; margin-top: 2px;">
                  Generated ${escapeHtml(synth.timestamp)} • Source: ${escapeHtml(synth.source || 'General')}
                </div>
              </div>
              <button class="btn-secondary" style="background: #FFF; color: #1A1A1A; border: none; font-size: 10px; padding: 4px 10px;" onclick="event.stopPropagation(); copyTextToClipboard(decodeURIComponent('${encodeURIComponent(synth.markdown)}'))">Copy Report</button>
            </div>
            <div id="synth-body-${idx}" class="synth-body" style="${idx === 0 ? 'display: block;' : 'display: none;'}">
